...
Upload metadata defaults live on `AppConfig.youtube_upload_defaults` (not credentials): category **Education**, `containsSyntheticMedia: true`, recording date = edition `publish_on` at NZ midnight, and best-effort `locationDescription: "New Zealand"`.
