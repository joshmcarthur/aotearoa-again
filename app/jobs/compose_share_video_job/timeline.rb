  ...
  class Timeline
    ...
    class << self
      def schedule_for(opts)
        ...
        hold_bw_end = hold_bw
        letterbox_end = hold_bw_end + letterbox
        wipe_end = letterbox_end + wipe
        end_at = wipe_end + hold_end
        caption_start = [ letterbox_end - caption_lead, hold_bw_end ].max
      ...
      def sample_at(time, schedule)
        layout_p, wipe_p =
          if time < schedule.hold_bw_end
            ...
            raw = (time - schedule.hold_bw_end) / (schedule.letterbox_end - schedule.hold_bw_end)
            [ ease_in_out_cubic(raw), 0.0 ]
          elsif time < schedule.wipe_end
            raw = (time - schedule.letterbox_end) / (schedule.wipe_end - schedule.letterbox_end)
            [ 1.0, smoothstep(raw) ]
          else
            [ 1.0, 1.0 ]
          end
