...
class ComposeShareImageJob
  module BrandChip
    ...
    def social_icons_row
      ...
      # Match web optical sizing: YouTube is wider than tall (~24×17).
      icons << icon_rgba(YOUTUBE_PATH, view_box: "0 3.5 24 17", width: (ICON_SIZE * 24 / 17.0).round, height: ICON_SIZE) if AppConfig.youtube_url.present?
      return if icons.empty?

      join_horizontal(icons, gap: 6)
    end

    def icon_rgba(path_d, view_box: "0 0 24 24", width: ICON_SIZE, height: ICON_SIZE)
      svg = <<~SVG
      ...
      if icon.width != width || icon.height != height
        icon = icon.thumbnail_image(width, height: height, size: :force)
      end
